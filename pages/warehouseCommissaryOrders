...
window.DMC_COMMISSARY_ORDERS_SELECTED_STATUS =
  window.DMC_COMMISSARY_ORDERS_SELECTED_STATUS || "all";

window.DMC_COMMISSARY_ORDERS_SELECTED_SOURCE =
  window.DMC_COMMISSARY_ORDERS_SELECTED_SOURCE || "all";

window.DMC_COMMISSARY_ORDERS_SEARCH = window.DMC_COMMISSARY_ORDERS_SEARCH || "";

window.DMC_COMMISSARY_ORDERS_DATE_FROM =
  window.DMC_COMMISSARY_ORDERS_DATE_FROM || getWarehouseOrderTodayDate();

window.DMC_COMMISSARY_ORDERS_DATE_TO =
  window.DMC_COMMISSARY_ORDERS_DATE_TO || getWarehouseOrderTodayDate();

window.DMC_COMMISSARY_ORDERS_SELECTED_ID =
  window.DMC_COMMISSARY_ORDERS_SELECTED_ID || "";

window.DMC_COMMISSARY_ORDER_FULFILLMENT_DRAFT =
  window.DMC_COMMISSARY_ORDER_FULFILLMENT_DRAFT || {};

function getStoredWarehouseOrders() {
  const storedOrders = localStorage.getItem(DMC_COMMISSARY_ORDERS_STORAGE_KEY);
...
function saveWarehouseOrders(orders) {
  localStorage.setItem(DMC_COMMISSARY_ORDERS_STORAGE_KEY, JSON.stringify(orders));
}
...
function getFilteredWarehouseOrders() {
  const status = window.DMC_COMMISSARY_ORDERS_SELECTED_STATUS;
  const source = window.DMC_COMMISSARY_ORDERS_SELECTED_SOURCE;
  const searchValue = String(window.DMC_COMMISSARY_ORDERS_SEARCH || "")
    .toLowerCase()
    .trim();

  const dateFrom = String(window.DMC_COMMISSARY_ORDERS_DATE_FROM || "");
  const dateTo = String(window.DMC_COMMISSARY_ORDERS_DATE_TO || "");

  return getSortedWarehouseOrders().filter((order) => {
    const orderSource = getOrderSource(order);

    const isCommissaryOrder =
      String(orderSource || "").toLowerCase().includes("commissary") ||
      String(order.department || "").toLowerCase() === "commissary";

    if (!isCommissaryOrder) {
      return false;
    }
...
function getSelectedWarehouseOrder() {
  ...
  if (window.DMC_COMMISSARY_ORDERS_SELECTED_ID) {
    const selectedOrder = orders.find(
      (order) => order.orderId === window.DMC_COMMISSARY_ORDERS_SELECTED_ID
    );
...
function renderWarehouseOrderStatusOptions() {
  const selectedStatus = window.DMC_COMMISSARY_ORDERS_SELECTED_STATUS;
...
function renderWarehouseOrderSourceOptions() {
  const selectedSource = window.DMC_COMMISSARY_ORDERS_SELECTED_SOURCE;
...
function getFulfillmentDraft(order) {
  ...
  if (!window.DMC_COMMISSARY_ORDER_FULFILLMENT_DRAFT[order.orderId]) {
    const lineDrafts = {};
    ...
    window.DMC_COMMISSARY_ORDER_FULFILLMENT_DRAFT[order.orderId] = {
      lines: lineDrafts,
  ...
  return window.DMC_COMMISSARY_ORDER_FULFILLMENT_DRAFT[order.orderId];
}
  ...
  if (orders.length === 0) {
    return `
      <div class="order-list-empty">
        ...
        <span>Commissary requests will appear here.</span>
      </div>
    `;
  }
        ...
        .map((order) => {
          ...
          return `
            <button
              ...
              data-select-commissary-order="${order.orderId}"
            >
              <div>
    ...
    <section class="panel branch-order-detail">
      ...
      <div class="branch-order-info-grid warehouse-order-info-grid">
        <div>
          ...
          <strong>${order.requestedBy || "Commissary Manager"}</strong>
        </div>
      ...
      <div class="branch-order-section">
        <div class="fulfillment-panel-header">
          <div>
            <h4>Prepare Commissary Order</h4>
            <p>Check Warehouse Stock, enter quantity to send, then send to Commissary.</p>
          </div>
...
function getCommissaryOrdersContent() {
  return `
    <section class="branch-orders-layout">
      <section class="panel branch-order-list-panel">
        <div class="panel-header">
          <div>
            <h3>Commissary Order Queue</h3>
            <p>Commissary requests waiting for Warehouse review and fulfillment.</p>
          </div>
        </div>
        ...
        >
          <label>
            ...
            <input
              ...
              type="date"
              value="${window.DMC_COMMISSARY_ORDERS_DATE_FROM}"
            />
          </label>
          ...
          <label>
            ...
            <input
              ...
              type="date"
              value="${window.DMC_COMMISSARY_ORDERS_DATE_TO}"
            />
          </label>
          ...
          <label style="grid-column: 1 / -1;">
            ...
            <input
              ...
              value="${window.DMC_COMMISSARY_ORDERS_SEARCH}"
            />
          </label>
...
function refreshCommissaryOrdersPage() {
  window.DMC_PAGES["warehouse-commissary-orders"].content = getCommissaryOrdersContent();
  renderPage("warehouse-commissary-orders");
}

function updateCommissaryOrderStatus(orderId, nextStatus, note, extraUpdates = {}) {
  const orders = getStoredWarehouseOrders();
  ...
  window.DMC_COMMISSARY_ORDERS_SELECTED_ID = orderId;
  refreshCommissaryOrdersPage();
}
...
function saveFulfillmentDraftFromInputs(order) {
  ...
  window.DMC_COMMISSARY_ORDER_FULFILLMENT_DRAFT[order.orderId] = draft;
}
...
function setupCommissaryOrdersEvents() {
  ...
  if (statusFilter) {
    statusFilter.addEventListener("change", () => {
      window.DMC_COMMISSARY_ORDERS_SELECTED_STATUS = statusFilter.value;
      window.DMC_COMMISSARY_ORDERS_SELECTED_ID = "";
      refreshCommissaryOrdersPage();
    });
  }
  ...
  if (sourceFilter) {
    sourceFilter.addEventListener("change", () => {
      window.DMC_COMMISSARY_ORDERS_SELECTED_SOURCE = sourceFilter.value;
      window.DMC_COMMISSARY_ORDERS_SELECTED_ID = "";
      refreshCommissaryOrdersPage();
    });
  }
  ...
  if (searchInput) {
    searchInput.addEventListener("input", () => {
      window.DMC_COMMISSARY_ORDERS_SEARCH = searchInput.value;
      window.DMC_COMMISSARY_ORDERS_SELECTED_ID = "";
      refreshCommissaryOrdersPage();
    });
  }
  ...
  if (dateFromInput) {
    dateFromInput.addEventListener("change", () => {
      window.DMC_COMMISSARY_ORDERS_DATE_FROM = dateFromInput.value;
      window.DMC_COMMISSARY_ORDERS_SELECTED_ID = "";
      refreshCommissaryOrdersPage();
    });
  }
  ...
  if (dateToInput) {
    dateToInput.addEventListener("change", () => {
      window.DMC_COMMISSARY_ORDERS_DATE_TO = dateToInput.value;
      window.DMC_COMMISSARY_ORDERS_SELECTED_ID = "";
      refreshCommissaryOrdersPage();
    });
  }
  ...
  if (clearFiltersButton) {
    clearFiltersButton.addEventListener("click", () => {
      window.DMC_COMMISSARY_ORDERS_SELECTED_STATUS = "all";
      window.DMC_COMMISSARY_ORDERS_SELECTED_SOURCE = "all";
      window.DMC_COMMISSARY_ORDERS_SEARCH = "";
      window.DMC_COMMISSARY_ORDERS_DATE_FROM = "";
      window.DMC_COMMISSARY_ORDERS_DATE_TO = "";
      window.DMC_COMMISSARY_ORDERS_SELECTED_ID = "";
      refreshCommissaryOrdersPage();
  ...
  document.querySelectorAll("[data-select-commissary-order]").forEach((button) => {
    button.addEventListener("click", () => {
      window.DMC_COMMISSARY_ORDERS_SELECTED_ID =
        button.dataset.selectCommissaryOrder;
  ...
  if (fulfillmentButton && selectedOrder) {
    fulfillmentButton.addEventListener("click", () => {
      updateCommissaryOrderStatus(
        selectedOrder.orderId,
        "Preparing",
        "Warehouse started preparing the Commissary order."
      );
    });
  }
  ...
  if (rejectButton && selectedOrder) {
    rejectButton.addEventListener("click", () => {
      ...
      updateCommissaryOrderStatus(
        selectedOrder.orderId,
        "Rejected",
        "Warehouse rejected the Commissary order request."
      );
    });
  }
  ...
  if (markOnTheWayButton && selectedOrder) {
    markOnTheWayButton.addEventListener("click", () => {
      ...
      const confirmed = confirm(
        `Send order ${selectedOrder.orderId} to Commissary? This will deduct Warehouse Stock and move it to Commissary Incoming Deliveries.`
      );
      ...
      updateCommissaryOrderStatus(
        selectedOrder.orderId,
        "On the Way",
        "Warehouse sent the order to Commissary. Warehouse Transfer Out was logged to the Ledger.",
        {
          fulfillment: {
            ...draft,
